...
pegarArmario indice armarios = case Map.lookup indice armarios of  
  ...
  Just (estado, codigo) -> if Data.Maybe.isJust (consulta indice armarios) 
...
aluguel indice armarios = case pegarCodigo indice armarios of
  Left string -> Left string
  Right codigoArm -> Right (Map.insert indice (Alugado, codigoArm) armarios)
...
devolucao indice codigo armarios = case pegarArmario indice armarios of
  Left string -> Left "oasjdoiasijdasd"
  Right (estadoArm, codigoArm) -> if estadoArm == Livre
  then Left "Armario nao esta alugado"
  else if codigoArm /= codigo
